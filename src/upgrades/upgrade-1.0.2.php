...
function upgrade_module_1_0_2($module)
{
    ...
    $indexes = Db::getInstance()->ExecuteS(
        'SHOW INDEX FROM `' . _DB_PREFIX_ . 'hiddenobjects_'.$module->prefix.'_founded`
        WHERE `column_name` = "ip_address"'
    );
    foreach ($indexes as $index) {
        if ($index['Key_name'] == 'id_hiddenobject') {
            $sql[] = 'ALTER TABLE '._DB_PREFIX_.'hiddenobjects_'.$module->prefix.'_founded DROP INDEX id_hiddenobject;';
        }
        if ($index['Key_name'] == 'id_hiddenobject_2') {
            $sql[] = 'ALTER TABLE '._DB_PREFIX_.'hiddenobjects_'.$module->prefix.'_founded DROP INDEX id_hiddenobject_2;';
        }
    }
